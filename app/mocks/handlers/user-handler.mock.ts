...
import { SignUpRequestBody, SignInRequestBody } from '@/app/business/user/user.type';
...
export const userHandlers = [
  ...
  http.post<never, SignInRequestBody, never>(`${API_PATH.auth}/sign-in`, async ({ request }) => {
    ...
    if (!isSuccess) {
      return HttpResponse.json({ status: 400, message: '아이디 또는 비밀번호가 일치하지 않습니다.' }, { status: 400 });
    }
